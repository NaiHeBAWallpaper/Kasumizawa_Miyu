...
let customScale = 1;
let targetFps = 60;
let bgmfile = './assets/audio/Cotton Candy Island.flac';
let bgmvolume = 0;
let bgm;
...
const AUDIO_DETAIL = [
	{ 
		...
		dialog: { 
				Japanese: [ 
					"呼吸を整えて…… 目標に、集中……。", 
					"距離1,150m、風は東北東から15ノット…… 温湿度による 誤差を補正……" 
				], 
				English: [ 
					"Phew... Take a breath... Focus on the target...", 
					"The distance is 1,150 meters...wind is from the northeast, fifteen knots. If I correct the error caused by temperature and humidity..." 
				], 
				Chinese: [ 
					"调整呼吸……专注目标……。", 
					"距离1150m，风速东北偏东 15 节…… 修正温度、湿度引起的误差……" 
				] 
			} 
	},
	{ 
		time: 19000, 
		count: 2, 
		startTimes: [ 1500, 14000 ], 
		dialog: { 
				Japanese: [ 
					"あ、あの……先生、できれば その、そういった悪戯はご遠慮いただけますと……", 
					"えっと、照準が乱れてしまうので……" 
				], 
				English: [ 
	...
	{ 
		time: 20000, 
		count: 2, 
		startTimes: [ 500, 15000 ], 
		dialog: { 
				Japanese: [ 
					"……なるほど。これは邪魔ではなく、実践に近い状況下での狙撃訓練、ということですよね……", 
					"SRTでも、似たような訓練をしたような気が……" 
				], 
				English: [ 
					"...Oh. I-It's not interference? Y-You're training me for a-actual combat...", 
					"I think w-we did this at SRT..." 
				], 
				Chinese: [ 
					"……原来如此。这不是妨碍是在接近实践的情况下进行狙击训练吧……", 
					"好像在SRT也进行了类似的训练……" 
				] 
			} 
	},
...
const HITBOX = {
	headpat: { xMin: 1050, xMax: 1800, yMin: 100, yMax: 550 },
	voiceline: { xMin: 634, xMax: 1880, yMin: 620, yMax: 1140 }
};
